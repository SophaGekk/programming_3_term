...
struct allocatorforme {// Тип данных, который может хранить аллокатор
    using value_type = T;
...
template <typename T, typename Allocator = allocatorforme<T>>
...
public:
    ...
    DoubleLinkedList(DoubleLinkedList&& other) noexcept
        : head(other.head), tail(other.tail), size(other.size), alloc(std::move(other.alloc)) {
        other.head = nullptr; 
        other.tail = nullptr;
...
int main() {
    ...
    std::map<int, int, std::less<int>, allocatorforme<std::pair<const int, int>, 10>> map2;
    ...
    LimitedContainer<std::pair<const int, int>, 10, allocatorforme<std::pair<const int, int>, 10>> limited_map; 
    ...
    // Создаем список с использованием `allocatorforme`
    DoubleLinkedList<int,allocatorforme<int>> list;
